# Additional Resources

## Table of Contents
- [Math Foundations](#math)
- [Programming/Databases](#prog)
- [Statistics](#stats)
- [Machine Learning](#ml)
- [Data Science Workflows and Presentation](#workflow)


## Math Foundations <a name = "math"></a>

### Books/Textbooks

### Websites/Articles/Slides

### Videos/Podcasts

### Online Courses


## Programming/Databases <a name = "prog"></a>

### Books/Textbooks
|Topic| Link | Description | Complexity | Cost |
|---  | ---  | ---         | ---        | ---  |

### Websites/Articles/Slides

|Topic| Link | Description | Complexity | Cost |
|---  | ---  | ---         | ---        | ---  |
|Programming (python, shell, R, etc.)| [Exercism.io](https://exercism.io/my/tracks) | Organized coding challenges with well defined tracks | Intro - Hard | Free |
| Shell Programming | [Learning the shell](http://linuxcommand.org/lc3_learning_the_shell.php) | Great introduction to learning the command line | Intro | Free |

### Videos/Podcasts

### Online Courses


## Statistics <a name = "stats"></a>

### Books/Textbooks
|Topic| Link | Description | Complexity | Cost |
|---  | ---  | ---         | ---        | ---  |
| Statistics | [Modern Dive](https://moderndive.com/) | Learning Inference in R | Intro | Free |
| Statistics | [Open Intro Statistics](https://leanpub.com/openintro-statistics) | Traditional textbook on foundational concepts in statistics | Intro | Suggested Donation $15 |
| Statistics | [An Introduction to the Bootstrap](https://www.crcpress.com/An-Introduction-to-the-Bootstrap/Efron-Tibshirani/p/book/9780412042317#googlePreviewContainer) | Study of computational techniques to understand complicated data sets | Intermediate | $125 |
| Statistics | [Statistical Rethinking - A Bayesian Course](https://www.amazon.com/Statistical-Rethinking-Bayesian-Examples-Chapman/dp/1482253445) | Computational Approach to statistical models | Intermediate | $90 |

### Websites/Articles

|Topic| Link | Description | Complexity | Cost |
|---  | ---  | ---         | ---        | ---  |
| A/B Testing | [Evans A/B Test Tools](https://www.evanmiller.org/ab-testing/) | Statistical tools/visualizations for A/B Tests | Intro | Free

### Videos/Podcasts

### Online Courses


## Machine Learning <a name = "ml"></a>

### Books/Textbooks
|Topic| Link | Description | Complexity | Cost |
|---  | ---  | ---         | ---        | ---  |
| Statistics | [An Introduction to Statistical Learning](http://faculty.marshall.usc.edu/gareth-james/ISL/) | Math and statistical foundations of Machine Learning | Advanced | Free |

### Websites/Articles

|Topic| Link | Description | Complexity | Cost |
|---  | ---  | ---         | ---        | ---  |
| Neural Nets | [A Neural Network Playground](http://playground.tensorflow.org/#activation=sigmoid&batchSize=10&dataset=spiral&regDataset=reg-plane&learningRate=0.1&regularizationRate=0&noise=0&networkShape=7&seed=0.44569&showTestData=false&discretize=false&percTrainData=50&x=true&y=true&xTimesY=true&xSquared=true&ySquared=true&cosX=false&sinX=true&cosY=false&sinY=true&collectStats=false&problem=classification&initZero=false&hideText=false) | Interactive Visualization to play with Neural Net hyper parameters and see how it converges on a prediction dataset | Intro | Free |
| ConvNet | [MNIST playground](http://scs.ryerson.ca/~aharley/vis/conv/flat.html)| Interactive Visualization where you draw a number and see how the layers of a CNN interpret your image | Intro | Free |
| ConvNet|[Intuitive explanation](https://ujjwalkarn.me/2016/08/11/intuitive-explanation-convnets/)| Explanation of ConvNet's and a good commentary on the MNIST playground resource | Intro | Free |
| Image Kernels | [Image Kernel Toggles](http://setosa.io/ev/image-kernels/)| Interactive Visualization to demonstrate how different kernels affect an image | Intro | Free |
| Recommender Systems | [Netflix Prize](https://medium.com/netflix-techblog/netflix-recommendations-beyond-the-5-stars-part-2-d9b96aa399f5) | Great Article detailing the 1 MM$ Netflix prize and the various considerations that the company has for recommending movies | Intro | Free |
| Recommender Systems | [Target predict pregnancy](https://www.forbes.com/sites/kashmirhill/2012/02/16/how-target-figured-out-a-teen-girl-was-pregnant-before-her-father-did/#4cbc50966668) | Article illustrating other considerations when recommending products (trust) | Intro | Free |
| Recommender Systems | [UBC Mark Schmidt Slides](https://www.cs.ubc.ca/~schmidtm/Courses/LecturesOnML/L29.pdf) | Overview slides on Content Based and Collaborative filtering (start at pg. 15) | Intermediate | Free|

### Videos/Podcasts

|Topic| Link | Description | Complexity | Cost |
|---  | ---  | ---         | ---        | ---  |
| Reinforcement Learning | [Vsauce - A Game that learns](https://www.youtube.com/watch?v=sw7UAZNgGg8) | Entertaining video on a simple reinforcement learning algorithm | Intro | Free |



## Data Science Workflows and Presentation <a name = "workflow"></a>

### Books/Textbooks
|Topic| Link | Description | Complexity | Cost |
|---  | ---  | ---         | ---        | ---  |
|Data Science Workflows | [The Art of Data Science](https://leanpub.com/artofdatascience) | Book describing the process of data science | Intro | Suggested donation $30 |

### Websites/Articles/Slides

|Topic| Link | Description | Complexity | Cost |
|---  | ---  | ---         | ---        | ---  |
|Data Visualization Principles | [Information Is Beautiful Awards](https://www.informationisbeautifulawards.com/) | Celebrates excellence & beauty in data visualization, infographics, interactive &  information art | Intro | Free |
| Data Visualization | [Data Viz Project](https://datavizproject.com/) | Compilation of many types of plots and examples | Intro | Free |
| Spatial Data Viz | [Kepler.gl](https://kepler.gl/) | geospatial analysis tool for large-scale data sets | Intro | Free |

### Videos/Podcasts

### Online Courses
